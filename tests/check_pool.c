...
START_TEST(test_start_submit_wait1)
{
	...
	struct threadpool_ops ops = { .alloc_worker = alloc_worker1,
				      ...
				      .do_work = do_work1 };
	int err;
	err = threadpool_new(&ops, 1, &pool);
	ck_assert_int_eq(err, 0);
	ck_assert(pool != NULL);
	err = threadpool_start(pool);
	ck_assert_int_eq(err, 0);
				   ...
				   struct threadpool_ops *ops)
{
	...
	err = threadpool_new(ops, nb_workers, &pool);
	ck_assert_int_eq(err, 0);
	ck_assert(pool != NULL);
	err = threadpool_start(pool);
	ck_assert_int_eq(err, 0);
...
static void dealloc_once(void *data)
{
	static int i = 0;
	i++;
...
{
	struct work_wait *e = list_entry(list, struct work_wait, list);
	int *d = (int*)data;
	for (int i = 0; i < 64; ++i)
		ck_assert_int_eq(d[i], i);
	return TPEMORE;
